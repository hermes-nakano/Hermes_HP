...
<!- 1列目 ->
				...
				<!- JavaScriptがOFFの場合 ->
					  			...
					  			<br>
					  			<FONT SIZE="6" COLOR="#000099">トレード掲示板</FONT><br><br>
	...
	　　トレード掲示板にようこそ！<br>
	<br>
	　　ある人には不用品でも、ある人にはお宝！そんな、ゴミに出される予定だったモノたちを、リサイクルしましょう！<br>
	<br>
	　　いらないモノがある人は、<b>　　【あげます】（物品名）</b>と題名に書いて、どんどん、出品しましょう！<br>
	    　　欲しいモノがある人は、<b>　　【欲しいです】（物品名）</b>と題名に書いて、どんどん、要請しましょう！<br>
	<br>
	　　もしかしたら、欲しかったあのモノが！？<br>
	　　ゴミは減り、地球環境にも貢献出来ます！ぜひ、積極的に利用しましょう！！<br>
	<br>
...
function _form_show($res_title="", $hidden="")
{
	...
	<div align="right">名前</font><font color="#ff0000">(必須)<input type="text" name="name" size="40" value="'.$v['name'].'" required></div>
	...
	<div align="right">本文<font color="#ff0000">(必須)</font><textarea name="mes" cols="42" rows="5" wrap="soft" required></textarea></div>
	<div style="position:absolute;">
	...
	<div align="right">投稿内容にかけるパスワード<font color="#ff0000">(必須)</font><input type="password" name="pass" size="8" value="'.$v['pass'].'" required></div>
	<div style="position:absolute;">
...
{
	global $colors,$icons; // $colors を _table 関数内で使えるように global 宣言。初期設定で指定し・ｽ $icons が _table 関数内で使え・驍謔､に global 宣言する。
...
function _lock()
{
	...
	// 10分たっていたら削除
	if (file_exists(DE_LOCKFILE)) { // ・烽ｵ、ロックファイル DE_LOCKFILE が・ｶ在していたら、処理を行います。
...
function _unlock()
{
	if (file_exists(DE_LOCKFILE)) rmdir(DE_LOCKFILE); // もし、ロックファイル DE_LOCKFILE が存在すれば、削除す・驕B
...
{
	// 許容サイズチェック
	if ($_FILES['upfile']['size'] > 1024000) _error_page('画像ファイルサイズが1024000バイトを超えています', $lock_flag); // アップロードされ・ｽファイルのサイズ $_FILES['upfile']['size'] が 102400 バイトを超えたら、_error_page関数を呼び出してエラーを表示する。
