...
<table id ="secret_table" cellspacing="0" cellpadding="0">
	<!-- コンテンツタイトル枠 -->
	<tr>
		<td colspan=4 class="m_bar" style="width:540px;">各書式ダウンロード</td>
	</tr>
	...
	<tr>
		<td></td>
		<td class="secret_cell4" colspan=2>
			クリックすると書式のダウンロードが行なえます。
		</td>
	</tr>
	...
	<!-- ファイル一覧読み込み -->
	<?php include "formatdl_files.php"; ?>
	...
	<?php for($i=0; $i < count($files); $i++){ ?>
		<tr>
			...
			<td class="secret_cell2">
				<?php if($files[$i]["url"]==''){ ?>
				<?php echo $files[$i]["title"]; ?>
				<?php }else{ ?>
				<a href="<?php echo $files[$i]["url"]; ?>" target="<?php echo $files[$i]["target"]; ?>" value="<?php echo $files[$i]["title"]; ?>"><?php echo $files[$i]["title"]; ?></a>
				<?php } ?>
			</td>
			<!-- ファイルサイズ -->
