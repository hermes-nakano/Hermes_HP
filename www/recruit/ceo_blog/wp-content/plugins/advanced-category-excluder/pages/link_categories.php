<?php
    ...
    if ($_POST['submit'] && !empty($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'link_categories')  )
          ...
          foreach ($ace_targets as $key=>$val)
      	  {
				...
				$s.=trim($v); if ($c != $k) $s.=",";
        		    ...
        		    update_option("ace_linkcategory_sections_".$key,$s);
        		}
        		// no, so we clean up.
        		else update_option("ace_linkcategory_sections_".$key,'');
    ...
    if ($_POST['empty'] && !empty($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'link_categories')  )
    {
    	foreach ($ace_targets as $key=>$val)
    	{
    	    update_option("ace_linkcategory_sections_".$key,"");
    	}
    }
...
<div class="wrap">
  ...
  <form method="post">
  <?php
    wp_nonce_field( 'link_categories', '_wpnonce', false );
  ?>
    <div id="ace_categories_usage" class="postbox">
      ...
      <div class="inside">
      	<ul>
      	 <li><?php echo sprintf(__("Here are %s different sections on the site. You can define section by section which link categories would you like to hide.","ace"), count($ace_targets)); ?></li>
      	</ul>
      </div>
    </div>
    <div class="postbox">
     ...
     <?php 
      foreach ($ace_targets as $key=>$val):
      $_cats = explode(",",get_option("ace_linkcategory_sections_".$key));
     ?>
