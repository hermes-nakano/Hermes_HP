<?php
    ...
    if ($_POST['submit']&& !empty($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'pages') )
	          ...
	          foreach ($ace_targets as $key=>$val)
	      	  {
					...
					$s.=trim($v); if ($c != $k) $s.=",";
	        		    ...
	        		    update_option("ace_page_sections_".$key,$s);
	        		}
	        		// no, so we clean up.
	        		else update_option("ace_page_sections_".$key,'');
    ...
    if ($_POST['empty'] && !empty($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'pages') )
    {
    	foreach ($ace_targets as $key=>$val)
    	{
    	    update_option("ace_page_sections_".$key,"");
    	}
    }
...
?>
<div class="wrap">
  <h2><?php _e('Advanced Category Excluder','ace'); ?>&nbsp;-&nbsp;<?php _e('Pages','ace'); ?></h2>
  <div class="metabox-holder" id="poststuff">
  <form method="post">
  <?php
    wp_nonce_field( 'pages', '_wpnonce', false );
  ?>
    ...
    <div class="postbox">
     ...
     <?php 
      foreach ($ace_targets as $key=>$val):
      $_cats = explode(",",get_option("ace_page_sections_".$key));
     ?>
