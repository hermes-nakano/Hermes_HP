<?php
    ...
    if ($_POST['submit'] && !empty($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'links')  )
    ...
    if ($_POST['empty'] && !empty($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'links')  )
    {
    	foreach ($ace_targets as $key=>$val)
    	{
...
<div class="wrap">
  ...
  <form method="post">
  <?php
    wp_nonce_field( 'links', '_wpnonce', false );    
  ?>
