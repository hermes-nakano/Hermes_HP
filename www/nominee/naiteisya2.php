...
$file = "data/naiteisya_h31.csv";	// 翌年入社の内定者情報の csvファイル
//$file_next = "data/naiteisya_h31.csv"; //翌翌年入社の内定者情報のcsvファイル
...
$counter = 0;
preg_match("#\w+/\w+(\d{2})#",$file,$matches);
$joined_year = $matches[1];
echo "<h1>平成{$joined_year}年春季採用内定者</h1>";
...
/*
$counter = 0; // 初期化
preg_match("#\w+/\w+(\d{2})#",$file_next,$matches);
$joined_year = $matches[1];
echo "<h1>平成{$joined_year}年春季採用内定者</h1>";
if ( ( $handle = fopen ( $file_next, "r" ) ) !== FALSE ) {
	$update_date = date("Y/m/d", filemtime($file));
	echo "<div>{$update_date} 更新</div>";
